      ...
      <el-tabs v-model="activeTabName" type="border-card" @tab-click="getList">
        ...
        <div style="margin-bottom:20px">
          ...
          <el-button-group class="margin-left-20" v-if="!isShowAddButton">
            <el-button type="primary" plain size="small" :disabled="tableData.length==0 || tableData[0].publishStatus>0" @click="setPublish(1)">公示</el-button>
            <el-button type="info" plain size="small" :disabled="tableData.length==0 || tableData[0].publishStatus<1" @click="setPublish(0)">取消</el-button>
          </el-button-group>
        </div>
        <el-container>
          <el-table
    ...
    getList() {
      ...
      Request()
        .get("/api/company_credit_grade/all", {
          pageNo: this.page.pageIndex - 1,   // approvalStatus: 2,
