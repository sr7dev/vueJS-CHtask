...
export default {
  ...
  methods: {
    getData() {
      ...
      Request()
        .get("/api/shareFiles/get/" + this.id)
        .then(response => {
          this.ruleFormValue.fileName = response.fileName;
          this.ruleFormValue.creater = response.creater;
          this.ruleFormValue.isVisible = response.isVisible;
          setTimeout(() => {
    ...
    onSubmit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          Request()
            .put("/api/shareFiles/update/" + this.id, {
