  ...
  <div class="container">
    <div class="title">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item class="actived">修改批次</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="box">
      <el-form
...
export default {
  ...
  created() {
    ...
    this.selectedId = this.$route.params.id1;
    this.getVarietyData();
    this.getList();
  },
  methods: {
    getList() {
      ...
      Request()
        .get("/api/product_batch/get/" + this.selectedId)
        .then(res => {
          this.rowData = res;
          this.ruleFormdata.varietyId = res.varietyId;
          this.ruleFormdata.varietyGrade = res.varietyGrade;
          setTimeout(() => {
    ...
    onSubmit(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.listLoading = true;
          formData.append("batchNumber", this.rowData.batchNumber);
          formData.append("createTime", this.rowData.createTime);
          formData.append("companyId", Storage.get("selectedCompanyId"));
          formData.append("id", this.rowData.id);
          formData.append("createUserId", this.rowData.createUserId);
          formData.append("productId", this.productId);
          formData.append("property", this.rowData.property);
          ...
          if (this.rowData.reportResult) {
            formData.append("reportResult", this.rowData.reportResult);
          }
          Request()
            .put("/api/product_batch/update/" + this.selectedId, formData)
            .then(response => {
              setTimeout(() => {
                this.listLoading = false;
              }, 0.5 * 1000);
